...
def build_vector_store(force_rebuild=False):
    ...
    try:
        ...
        try:
            ...
            print(
                f"✅ Embeddings initialized successfully (dimension: {len(test_embedding)})"
            )
        except Exception as e:
        ...
        if os.path.exists(vector_store_path):
            try:
                ...
                if e.errno == 16:  # Device or resource busy
                    print(
                        "⚠️ Vector store directory locked (Docker volume), clearing contents instead..."
                    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Pre-build vector store for RAG chatbot"
    )
    parser.add_argument(
        "--force",
        action="store_true",
