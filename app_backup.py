...
def show_debug_page():
    ...
    if st.session_state.get("token_usage"):
        ...
        if usage["session_history"]:
            ...
            if len(usage["session_history"]) > 1:
                ...
                input_tokens = [
                    entry["input_tokens"] for entry in usage["session_history"]
                ]
                output_tokens = [
                    entry["output_tokens"] for entry in usage["session_history"]
                ]
    ...
    st.info(
        "💡 **Tip**: Reducing max context tokens will lower input token usage but may reduce answer quality."
    )
                ...
                try:
                    ...
                    if os.path.exists(vector_store_path):
                        ...
                        if count > 0:
                            ...
                            st.success(
                                f"✅ Query test successful - found {len(docs)} results"
                            )
                        else:
...
# Navigation
page = st.sidebar.selectbox("Select Page", ["💬 Chat", "🔧 Debug Info"], index=0)
